...
@section('content')
    <nav class="breadcrumb">
        <i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 分类管理 <span class="c-gray en">&gt;</span> 菜谱分类
        <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新"><i class="Hui-iconfont">&#xe68f;</i></a>
        <a class="btn btn-success radius r" style="line-height:1.6em;margin:3px 5px 0 0" href="{{url('admin/caipu_category/create')}}" title="添加"><i class="Hui-iconfont">&#xe600;</i></a>
    </nav>
    <style>
        .Huifold .item {
            position: relative
                    ...
                    <li class="item">
                        <h4 style="overflow: hidden">
                            ...
                            <span style="position: absolute;right: 30px;">
                                <a href="{{url('admin/caipu_category/'.$v->cate_id.'/edit')}}" class="Hui-iconfont">&#xe647;修改</a>
                            ...
                            @foreach($data_p as $n)
                                @if($n->cate_pid == $v->cate_id)
                                        <li style="height: 35px">　
                                            ...
                                            <span style="position: absolute;right: 30px;">
                                                <a href="{{url('admin/caipu_category/'.$n->cate_id.'/edit')}}" class="Hui-iconfont">&#xe647;修改</a>
...
@section('script')
    <script>
        ...
        function changeorder(obj, cate_id) {
            ...
            $.post("{{url('admin/caipu_category/changeorder')}}", {
                '_token': '{{csrf_token()}}',
                'cate_id': cate_id,
        ...
        function delCate(cate_id) {
            ...
            layer.confirm('您确定要删除该分类？', {
                ...
                //删除
                $.post("{{url('admin/caipu_category/')}}/" + cate_id, {
                    '_token': '{{csrf_token()}}',
                    '_method': 'delete'
                }, function (data) {
                    if (data.status == 0) {
